...
def voc_eval(detpath,
    ...
    sorted_scores = np.sort(-confidence)#对confidence的index根据值大小进行降序排列。
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            overlaps = inters / uni
            ovmax = np.max(overlaps)#最大重合度值
            jmax = np.argmax(overlaps)#最大重合度值得所在位置

        if ovmax > ovthresh:
            if not R['difficult'][jmax]:#R['difficult'][jmax]初始都为0
                if not R['det'][jmax]:#R['det][jmax]初始都为0
                    tp[d] = 1.#R['difficult'][jmax]初始都为0
                    R['det'][jmax] = 1#该gt被置为已检测到，下一次若还有另一个检测结果与之重合率满足阈值，则不能认为多检测到一个目标，fp加1
                else:
                    fp[d] = 1.
        else:
            fp[d] = 1.
    ...
    fp = np.cumsum(fp)# 非目标预测为目标与 预测正确但重复数量
    tp = np.cumsum(tp) # 预测准确的框数
    rec = tp / float(npos)# npos实际标签数，查全率
    # avoid divide by zero in case the first detection matches a difficult
    # ground truth
    prec = tp / np.maximum(tp + fp, np.finfo(np.float64).eps) # np.finfo(np.float64).eps浮点型最小正数，查准率
    ap = voc_ap(rec, prec, use_07_metric)
